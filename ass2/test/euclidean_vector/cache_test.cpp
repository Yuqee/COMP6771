...
TEST_CASE("norm") {
	...
	for (auto i = 0; i < dim; ++i) {
		auto nrom_val = comp6771::euclidean_norm(vec1);
		CHECK(abs(nrom_val - sqrt(dim)) < EPSILON);
	}
	auto num = double{3};
	vec1 *= num;
...
TEST_CASE("norm cache +=") {
	...
	for (auto i = 0; i < epoch; ++i) {
		vec1 += zeros;
		auto norm_val = comp6771::euclidean_norm(vec1);
		CHECK(abs(norm_val - sqrt(dim)) < EPSILON);
	}
}
